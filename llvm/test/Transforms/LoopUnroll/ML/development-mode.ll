...
; RUN: %python %t.rundir/interactive_main.py 5 no %t.nchannel-basename \
...
; RUN: %python %t.rundir/interactive_main.py 0 no %t.nchannel-basename \
; RUN:    opt %S/Inputs/nested.ll -S -O3 --mlgo-loop-unroll-interactive-channel-base=%t.nchannel-basename \
; RUN:    --mlgo-loop-unroll-advisor-mode=development \
; RUN:    --interactive-model-runner-echo-reply \
; RUN:    -debug-only=loop-unroll-development-advisor \
...
;
; RUN: %python %t.rundir/interactive_main.py -1 no %t.nchannel-basename \
; RUN:    opt %S/Inputs/nested.ll -S -O3 --mlgo-loop-unroll-interactive-channel-base=%t.nchannel-basename \
; RUN:    --mlgo-loop-unroll-advisor-mode=development \
; RUN:    --interactive-model-runner-echo-reply \
; RUN:    -debug-only=loop-unroll-development-advisor \
; RUN:    -o /dev/null 2>&1 | FileCheck %s --check-prefix=CHECK3

; RUN: rm -rf %t.rundir/instrumented-module.ll
; RUN: %python %t.rundir/interactive_main.py -1 instrument %t.nchannel-basename \
; RUN:    opt %S/Inputs/nested.ll -S -O3 --mlgo-loop-unroll-interactive-channel-base=%t.nchannel-basename \
; RUN:    --mlgo-loop-unroll-advisor-mode=development \
; RUN:    --interactive-model-runner-echo-reply \
; RUN:    -debug-only=loop-unroll-development-advisor \
...
;
; RUN: cat %t.rundir/instrumented-module.ll | FileCheck %s --check-prefix=CHECK4
...
; CHECK3: mlgo-loop-unroll: got advice nodecision
; CHECK3: mlgo-loop-unroll: unattempted unroll
; CHECK3: mlgo-loop-unroll: got advice nodecision
; CHECK3: mlgo-loop-unroll: unattempted unroll
; CHECK3: mlgo-loop-unroll: got advice nodecision
; CHECK3: mlgo-loop-unroll: unattempted unroll
...
; CHECK4: test_loop_begin
; CHECK4: test_loop_end
